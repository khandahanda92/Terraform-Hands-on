...
data "aws_availability_zones" "available" {
  state = "available" # this will get all the availablity zone 
}
...
resource "aws_vpc" "levelup_vpc" {
  ...
  tags = {
    Name = "${var.ENVIRONMENT}-vpc"  # this is dependent on variable i,e if evn= prod then tag is prod-vpc
  }
}
...
resource "aws_subnet" "levelup_vpc_public_subnet_2" {
  ...
  availability_zone = data.aws_availability_zones.available.names[1] # here the availablity zne is different 
  map_public_ip_on_launch = "true"
  tags = {
...
resource "aws_subnet" "levelup_vpc_private_subnet_2" {
  ...
  availability_zone = data.aws_availability_zones.available.names[1] # here also the az is different 
  tags = {
...
resource "aws_eip" "levelup_nat_eip" {
  vpc      = true
  depends_on = [aws_internet_gateway.levelup_igw] # depend on the internet gateway 
}

# NAT gateway for private ip address
resource "aws_nat_gateway" "levelup_ngw" {
  allocation_id = aws_eip.levelup_nat_eip.id
  subnet_id     = aws_subnet.levelup_vpc_public_subnet_1.id # this NAT gateway is in public subnet 1
  depends_on = [aws_internet_gateway.levelup_igw] # it will depend on the internet gateway 
  tags = {
