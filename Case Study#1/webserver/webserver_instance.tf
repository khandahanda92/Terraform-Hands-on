...
# this will create the vpc and rds from these tow modules 
...
resource "aws_security_group" "levelup_webservers"{
  ...
  ingress { # this is for creating the ssh connection 
    from_port = 22
    to_port = 22
    protocol = "tcp"
  ...
  ingress { # we will accept the http traffic 
    from_port = 80
    to_port = 80
    protocol = "tcp"
...
resource "aws_autoscaling_group" "levelup_webserver" {
  ...
  vpc_zone_identifier       = ["${module.levelup-vpc.levelup_vpc_public_subnet_1.id}", "${module.levelup-vpc.levelup_vpc_public_subnet_2.id}"]
...
resource "aws_lb" "levelup-load-balancer" {
  ...
  subnets            = ["${module.levelup-vpc.public_subnet1.id}", "${module.levelup-vpc.public_subnet2.id}"]
